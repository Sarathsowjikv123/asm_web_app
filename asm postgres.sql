...
-- Started on 2025-01-15 11:57:54 IST
...
--
-- TOC entry 244 (class 1255 OID 16863)
...
--
-- TOC entry 245 (class 1255 OID 16869)
-- Name: allocate_asset(integer, integer); Type: PROCEDURE; Schema: public; Owner: postgres
--

CREATE PROCEDURE public.allocate_asset(IN a_user_id integer, IN a_asset_id integer, OUT result_message text)
    LANGUAGE plpgsql
    AS $$
DECLARE 
...
BEGIN
	...
	IF NOT EXISTS (SELECT 1 FROM user_asset_mapping WHERE asset_id = a_asset_id 
		AND user_type_id = (SELECT user_type_id FROM users WHERE user_id = a_user_id)) THEN
			result_message := 'This Asset is Not Available For Your Role';
			RETURN;
	END IF;
	...
	IF EXISTS (SELECT 1 FROM asset_assignments_summary
		...
		result_message := 'Asset Already Allocated !!!';
		RETURN;
	END IF;
    ...
    result_message := 'Asset Allocated Successfully';
END;
$$;


ALTER PROCEDURE public.allocate_asset(IN a_user_id integer, IN a_asset_id integer, OUT result_message text) OWNER TO postgres;

--
-- TOC entry 247 (class 1255 OID 16815)
...
--
-- TOC entry 250 (class 1255 OID 16864)
-- Name: raise_request(integer, integer, character varying); Type: PROCEDURE; Schema: public; Owner: postgres
--

CREATE PROCEDURE public.raise_request(IN new_user_id integer, IN new_asset_id integer, IN new_operation character varying, OUT status_message text, OUT success boolean)
    LANGUAGE plpgsql
    AS $$
DECLARE
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM assets WHERE asset_id = new_asset_id) THEN
        status_message := 'INVALID ASSET-ID !!!';
        RETURN;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM user_asset_mapping WHERE asset_id = new_asset_id 
        ...
        status_message := 'This Asset is Not Available For Your Role';
        RETURN;
    END IF;
    ...
    IF new_operation = 'ASSIGN' AND EXISTS (SELECT 1 FROM asset_assignments_summary
            ...
            AND is_active = TRUE
            AND completed_date_time IS NOT NULL) THEN
        status_message := 'Asset Already Allocated !!!';
        RETURN;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM asset_assignments_summary WHERE user_id = new_user_id AND
            ...
            is_active = FALSE AND
            completed_date_time IS NULL ) THEN
        status_message := 'Request Already Raised !!!';
        RETURN;
    END IF;
    ...
    IF new_operation = 'RETAIN' AND NOT EXISTS (SELECT 1 FROM asset_assignments_summary
            ...
            AND is_active = TRUE
            AND completed_date_time IS NOT NULL) THEN
        status_message := 'This Asset is Not Allocated To This User!!!';
        RETURN;
    END IF;

    -- Insert into asset_assignments_summary
    ...
    success := TRUE;
    status_message := 'Request Raised Successfully';
END;
$$;


ALTER PROCEDURE public.raise_request(IN new_user_id integer, IN new_asset_id integer, IN new_operation character varying, OUT status_message text, OUT success boolean) OWNER TO postgres;

--
-- TOC entry 249 (class 1255 OID 16830)
...
--
-- TOC entry 246 (class 1255 OID 16870)
-- Name: retain_asset(integer, integer); Type: PROCEDURE; Schema: public; Owner: postgres
--

CREATE PROCEDURE public.retain_asset(IN r_user_id integer, IN r_asset_id integer, OUT status_message text)
    LANGUAGE plpgsql
    AS $$
DECLARE
	req_assignment_id INT;
BEGIN
	...
	IF NOT EXISTS (SELECT 1 FROM asset_assignments_summary
			...
			AND is_active = TRUE
			AND completed_date_time IS NOT NULL) THEN
		status_message := 'Asset Not Allocated !!!';
	return;
	END IF;
...
ALTER PROCEDURE public.retain_asset(IN r_user_id integer, IN r_asset_id integer, OUT status_message text) OWNER TO postgres;

--
-- TOC entry 248 (class 1255 OID 16817)
...
56	26	29	ASSIGN	t	2025-01-09 10:47:57.673553
57	23	31	ASSIGN	t	2025-01-09 10:51:54.5525
59	23	32	RETAIN	f	2025-01-09 11:29:31.929076
58	23	32	ASSIGN	f	2025-01-09 11:29:31.929076
60	23	27	RETAIN	f	2025-01-09 11:29:31.938171
54	23	27	ASSIGN	f	2025-01-09 11:29:31.938171
64	27	31	RETAIN	f	2025-01-10 17:38:47.609864
62	27	31	ASSIGN	f	2025-01-10 17:38:47.609864
66	27	31	RETAIN	f	2025-01-10 17:39:14.792172
65	27	31	ASSIGN	f	2025-01-10 17:39:14.792172
70	27	31	RETAIN	f	2025-01-10 17:44:31.432778
67	27	31	ASSIGN	f	2025-01-10 17:44:31.432778
72	27	31	RETAIN	f	2025-01-10 17:44:44.751482
71	27	31	ASSIGN	f	2025-01-10 17:44:44.751482
73	27	31	ASSIGN	t	2025-01-10 17:51:39.739673
75	24	32	RETAIN	f	2025-01-14 17:02:10.442828
74	24	32	ASSIGN	f	2025-01-14 17:02:10.442828
77	24	32	RETAIN	f	2025-01-14 17:05:19.819444
76	24	32	ASSIGN	f	2025-01-14 17:05:19.819444
78	27	30	RETAIN	f	2025-01-15 10:26:30.936813
63	27	30	ASSIGN	f	2025-01-15 10:26:30.936813
80	27	33	RETAIN	f	2025-01-15 10:30:22.752004
79	27	33	ASSIGN	f	2025-01-15 10:30:22.752004
83	27	29	RETAIN	f	2025-01-15 10:33:57.310304
82	27	29	ASSIGN	f	2025-01-15 10:33:57.310304
84	27	33	RETAIN	f	2025-01-15 10:45:26.538175
81	27	33	ASSIGN	f	2025-01-15 10:45:26.538175
86	27	29	RETAIN	f	2025-01-15 10:53:38.547812
85	27	29	ASSIGN	f	2025-01-15 10:53:38.547812
88	27	33	RETAIN	f	2025-01-15 11:01:31.771567
87	27	33	ASSIGN	f	2025-01-15 11:01:31.771567
90	27	33	RETAIN	f	2025-01-15 11:04:51.389743
89	27	33	ASSIGN	f	2025-01-15 11:04:51.389743
92	24	33	RETAIN	f	2025-01-15 11:07:59.995522
61	24	33	ASSIGN	f	2025-01-15 11:07:59.995522
93	24	27	RETAIN	f	2025-01-15 11:08:00.004534
55	24	27	ASSIGN	f	2025-01-15 11:08:00.004534
94	24	32	RETAIN	f	2025-01-15 11:08:00.00695
91	24	32	ASSIGN	f	2025-01-15 11:08:00.00695
97	24	27	RETAIN	f	2025-01-15 11:10:39.738353
96	24	27	ASSIGN	f	2025-01-15 11:10:39.738353
98	24	32	RETAIN	f	2025-01-15 11:10:39.748702
95	24	32	ASSIGN	f	2025-01-15 11:10:39.748702
101	24	27	RETAIN	f	2025-01-15 11:13:26.85135
99	24	27	ASSIGN	f	2025-01-15 11:13:26.85135
102	24	32	RETAIN	f	2025-01-15 11:13:26.857047
100	24	32	ASSIGN	f	2025-01-15 11:13:26.857047
103	27	30	ASSIGN	t	2025-01-15 11:38:14.437779
\.
...
27	Phone	t	HARDWARE
29	Laptop	t	HARDWARE
30	Charger	t	HARDWARE
31	I-Phone	t	HARDWARE
32	Monitor	t	HARDWARE
33	Charger	t	HARDWARE
\.
...
24	62	2025-01-10 17:21:40.086312
25	63	2025-01-10 17:22:57.54716
26	76	2025-01-14 17:04:53.323572
\.
...
SELECT pg_catalog.setval('public.asset_assignments_summary_assignment_id_seq', 103, true);
...
SELECT pg_catalog.setval('public.current_assets_asset_id_seq', 33, true);
...
SELECT pg_catalog.setval('public.current_users_user_id_seq', 27, true);
...
SELECT pg_catalog.setval('public.request_table_request_id_seq', 26, true);
...
-- Completed on 2025-01-15 11:57:54 IST
